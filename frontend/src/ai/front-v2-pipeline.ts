...
// INLINED FROM METADATA JSON - Bypasses the need for feature-schema.ts
const FEATURE_NAMES = [
  "left_ear", "right_ear", "avg_ear", "normalized_left_ear", "normalized_right_ear",
  "normalized_avg_ear", "ear_rolling_mean", "ear_rolling_min", "ear_rolling_std",
  "ear_slope", "continuous_eye_closed_sec", "iris_x_ratio", "iris_y_ratio",
  "gaze_x_delta_from_baseline", "gaze_y_delta_from_baseline", "gaze_x_rolling_mean",
  "gaze_y_rolling_mean", "gaze_x_rolling_std", "gaze_y_rolling_std",
  "face_head_down_ratio", "face_head_tilt_ratio", "face_head_down_delta",
  "face_head_tilt_delta", "pose_head_drop_ratio", "pose_head_tilt_ratio",
  "pose_head_drop_delta", "pose_head_tilt_delta", "shoulder_slope",
  "shoulder_slope_delta", "face_seen", "pose_seen", "face_valid_ratio",
  "pose_valid_ratio", "calibration_valid"
] as const;

const BASELINE_KEYS = [
  "leftEar", "rightEar", "avgEar", "irisXRatio", "irisYRatio",
  "faceHeadDownRatio", "faceHeadTiltRatio", "poseHeadDropRatio",
  "poseHeadTiltRatio", "shoulderSlope",
] as const;
...
export class FrontV2FeaturePipeline {
  ...
  process(timestampMs: number, measurements: FrontMeasurements): FrontV2FeatureResult {
    ...
    const missingFeatures = FEATURE_NAMES.filter((name) => {
      const value = values[name];
      return value === null || !Number.isFinite(value);
    });

    // Generate the Float32Array manually
    let vector: Float32Array | null = null;
    if (missingFeatures.length === 0) {
       vector = new Float32Array(34);
       for (let i = 0; i < 34; i++) {
           vector[i] = values[FEATURE_NAMES[i]] as number;
       }
    }
